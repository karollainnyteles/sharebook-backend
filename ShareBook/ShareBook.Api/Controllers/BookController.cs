...
public class BookController : ControllerBase
{
    ...
    public PagedList<BookVMAdm> Paged(int page, int items)
    {
        ...
        var books = _service.Get(x => x.Title, page, items,
    ...
    {
        var winnerUserId = new Guid(Thread.CurrentPrincipal?.Identity?.Name);
    ...
    public IActionResult Get(string slug)
    {
        ...
        return book != null ? Ok(bookVM) : NotFound();
    }
    ...
    public IActionResult GetById(string id)
    {
        ...
        return bookVM != null ? Ok(bookVM) : NotFound();
    }
    ...
    {
        var user = GetUser();
        if (_IsDonator(requestBookVM.BookId, user) &&
            !_IsAdmin(user)) //Permitido solicitar o próprio livro somente para Admin
    ...
    public IActionResult CancelRequest(Guid requestId)
    {
        ...
        var success = _bookUserService.CancelRequest(request);
        if (!success)
            return BadRequest();
    ...
    public async Task<IActionResult> MainUsers(Guid bookId)
    {
        ...
        VisitorProfile GetVisitorProfile(MainUsersVM mainUsers)
        {
            ...
            if (mainUsers.Facilitator is not null) facilitatorId = mainUsers.Facilitator.Id;

            var winnerId = Guid.Empty;
            if (mainUsers.Winner is not null) winnerId = mainUsers.Winner.Id;

            var donorId = Guid.Empty;
            if (mainUsers.Donor is not null) donorId = mainUsers.Donor.Id;
            ...
            if (visitor.Id.Equals(winnerId)) return VisitorProfile.Winner;
            if (visitor.Id.Equals(donorId)) return VisitorProfile.Donor;
