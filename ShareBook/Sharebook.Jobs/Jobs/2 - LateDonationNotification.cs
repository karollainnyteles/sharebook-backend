...
public class LateDonationNotification : GenericJob, IJob
{
    ...
    private readonly int _maxLateDonationDays;
        ...
        IEmailTemplate emailTemplate, IConfiguration configuration) : base(jobHistoryRepo)
    {
        ...
        _maxLateDonationDays = int.Parse(configuration["SharebookSettings:MaxLateDonationDays"]);
    }
    ...
    private void SendEmailAdmin(IList<Book> booksLate, BookStatsDto status)
    {
        ...
        stringBuilder.Append("<TABLE border=1 cellpadding=3 cellspacing=0><TR bgcolor='#ffff00'><TD><b>LIVRO</b></TD><TD><b>DIAS DE <BR>ATRASO</b></TD><TD><b>TOTAL <br>INTERESSADOS</b></TD><TD><b>DOADOR</b></TD><TD><b>FACILITADOR</b></TD><TD><b>ANOTAÇÕES</b></TD></TR>");
        ...
        foreach (var book in booksLate)
        {
            ...
            stringBuilder.Append(string.Format("<TR><TD>{0}<BR>{1}</TD><TD>{2}</TD><TD>{3}</TD><TD>{4}<BR>{5}<BR>{6}<BR>{7}</TD><TD>{8}<BR>{9}<BR>{10}<BR>{11}</TD><TD>{12}</TD></TR>",
                book.Title,
                book.Status,
                book.DaysLate(),
        ...
        var htmlTable = stringBuilder.ToString();
    ...
    private void SendEmailDonators(IList<User> donators, ref string details)
    {
        foreach (var donator in donators)
        {
            ...
            if (donator.HasAbandonedDonation(_maxLateDonationDays))
    ...
    {
        var html = $"<p>Bom dia. Consta em nosso sistema que você tem uma doação abandonada no sharebook com mais de {_maxLateDonationDays} dias de atraso.</p>";
